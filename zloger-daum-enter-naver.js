...
(async () => {
  ...
  // await checkExecutionTime('zloger_daum_entertainment_time_check.json', 2);
  ...
  // const fs = require('fs');
  // const newsList = JSON.parse(fs.readFileSync('./data/zloger_daum_entertainment_data.json', 'utf-8'));

  // 외부 URL에서 newsList 데이터 가져오기 (github raw)
  const NEWS_JSON_URL = 'https://raw.githubusercontent.com/ggpt6choi-coder/blogman/main/data/zloger_daum_entertainment_data.json';
  ...
  for (let i = 0; i < newsList.length; i++) {
    ...
    const blogData = {
      page,
      blogName: process.env.BLOG_NAME_JI_2,
      title: news.newTitle,
