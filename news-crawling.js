...
(async () => {
  ...
  for (const sc of scList) {
    ...
    for (const link of links) {
      ...
      if (article !== '[본문 없음]') {
        try {
          const prompt = `다음 뉴스 본문을 네이버 블로그 검색 엔진에 최적화된 본문으로 재가공해줘. 조건은 아래와 같아.\n\n
            ...
            - 글자 수는 1000자 이상 1500자 미만으로 맞춰줘.\n\n
            - 소제목(h2, h3)을 달아 가독성을 높이고, 소제목 단위로 문단을 나눠서 작성해줘. 소제목 앞에는 "✅" 기호를 붙여줘.\n\n
            - 맨 처음에는 너의 중립적인 생각을 짧게 말해주고, 한 문단 띄운 뒤 "무슨 내용인지 보러 가시죠!"라는 문장을 넣어줘.\n\n
      ...
      if (newArticle !== '[본문 없음]' && newTitle !== '[제목 없음]') {
        newsArr.push({
          type: sc,
          title,
          newTitle,
