        ...
        for (const link of links) {
            ...
            if (article !== '[본문 없음]' && article.length !== 0 && title !== '[제목 없음]') {
                try {
                    ...
                    주어진 뉴스 기사를 재료로 삼아, 검색 사용자가 궁금해할 정보를 해소해주고 내 의견을 덧붙여 체류 시간을 길게 가져가는 포스팅 데이터를 생성해줘.

                    [필수 출력 포맷: JSON]
                    {
                        "searchKeywords": ["메인키워드", "서브키워드1", "서브키워드2"],
                        "newTitle": "클릭을 부르는 매력적인 제목 (특수문자 제외)",
                        "newArticle": [
                            {
                                "title": "감성적이고 호기심을 자극하는 소제목",
                                "content": "독자에게 말을 거는 듯한 도입부 내용 (300자 이상)"
                            },
                            {
                                "title": "핵심 요약 및 팩트 체크",
                                "content": "기사의 핵심 내용이나 사건의 흐름을 파악하기 쉽게 풀어쓴 요약글 (줄글 형태)"
                            },
                            {
                                "title": "심층 분석 및 배경 설명",
                                "content": "뉴스 내용을 상세하게 풀어쓴 본문 (400자 이상)"
                            },
                            {
                                "title": "솔직한 후기 및 나만의 시선",
                                "content": "단순 정보를 넘어선 나의 생각과 다짐 (300자 이상)"
                            }
                        ],
                    ...
                    - newTitle: [메인 키워드]를 반드시 포함하되, '궁금증 유발' + '이득 제시' 구조로 작성. 
                    - 제목 금지 사항: 따옴표("), 대괄호([]), 마침표(.) 절대 금지. 느낌표(!)와 물음표(?)만 허용.

                    [핵심 전략 2: 다이아몬드(DIAMOND) 로직 대응]
                    - 단순 정보 전달이 아닌 '나의 생각', '반응', '예시'를 중심으로 재구성해. (AI가 쓴 티가 나지 않게)
                    - 문체: "저도 이거 보고 깜짝 놀랐는데요", "솔직히 말하면~", "이런 분들은 꼭 보세요!" 같은 **100% 구어체(수다 떠는 말투)** 사용.
                    - 금지 어미: "~습니다", "~합니다", "~이다" 절대 금지. -> "~했거든요", "~더라고요", "~인 것 같아요" 사용.

                    [작성 조건 상세]
                    1. newTitle: 25자 내외. 낚시성은 배제하되 클릭하고 싶게 만드세요.
                    
                    2. newArticle 구조 (순서 준수, 모든 필드는 문자열):
                        - 섹션 1 (Hook): 뉴스 요약 금지. "여러분, 이 소식 들으셨나요?" 처럼 상황을 제시하며 감정적으로 접근.
                        - 섹션 2 (Summary): 기사의 핵심 내용 3~4가지를 자연스럽게 연결하여 **하나의 문단**으로 요약 설명. (개조식/리스트 사용 금지, 줄글로 작성)
                        - 섹션 3 (Detail): 기사의 육하원칙을 풀어서 설명하되, 어려운 용어는 쉽게 순화. 배경지식을 덧붙여 풍성하게.
                        - 섹션 4 (Insight): "앞으로 저는 이렇게 하려고요" 같은 행동 유도나 주관적인 평가.

                    3. 분량: 전체 글자 수 공백 제외 1,500자 이상. 각 content는 호흡을 길게 가져가세요.
