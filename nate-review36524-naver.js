...
async function naverLogin(page) {
  ...
  await page.fill('#id', process.env.NAVER_ID_REVIEW36524);
  await page.fill('#pw', process.env.NAVER_PW_REVIEW36524.replace(/"/g, ''));
...
(async () => {
  ...
  const TIME_CHECK_URL = 'https://raw.githubusercontent.com/ggpt6choi-coder/blogman/main/data/review36524_nate_time_check.json';
  ...
  // const fs = require('fs');
  // const newsList = JSON.parse(fs.readFileSync('./data/review36524_nate.json', 'utf-8'));

  // 외부 URL에서 newsList 데이터 가져오기 (github raw)
  const NEWS_JSON_URL = 'https://raw.githubusercontent.com/ggpt6choi-coder/blogman/main/data/review36524_nate.json';
  ...
  for (let i = 0; i < newsList.length; i++) {
    ...
    const blogData = {
      page,
      blogName: process.env.BLOG_NAME_REVIEW36524,
