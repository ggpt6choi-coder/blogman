        ...
        try {
            ...
            if (article !== '[본문 없음]' && article.length !== 0 && title !== '[제목 없음]') {
                try {
                    ...
                    const prompt = `
                        너는 네이버 블로그 검색 로직(Smart Block, Air Search)을 뚫을 수 있는 '상위 1% 연예 블로거'야.
                        주어진 기사를 재료로, **검색 결과 1위와 네이버 메인 노출**을 동시에 잡을 수 있는 포스팅을 작성해.

                        [🔴 적용 페르소나: "${selectedConcept}"]
                        - 기계적인 말투 금지. 팬심과 감정을 200% 담아서 작성.
                        - **볼드체나 특수기호로 멋을 부리기보다, 흡입력 있는 문장력으로 승부해.**
                        ...
                        - **제3원칙 (필수): 특수문자(!, ?, [], (), ~), 이모지, 한자 절대 사용 금지.** 오직 **한글, 숫자, 띄어쓰기**만 사용할 것.
                        - (나쁜 예): "[충격] 숙행 상간 소송?? (대박)" (특수문자 포함됨)
                        - (완벽한 예): "숙행 상간 소송 의혹 사건반장 40대 여가수 지목에 현역가왕3 비상"

                        [Step 2. 본문 도입부(SEO) 전략 - '노출'의 핵심]
                        - **첫 문장 규칙:** 본문 시작 **첫 문장**에 반드시 **메인 키워드**를 자연스럽게 포함시킬 것. (검색 로봇 인식용)
                        - 예시: "여러분, 오늘 터진 **숙행 상간 소송** 의혹 보셨나요? 진짜 저 너무 놀랐잖아요!"

                        [Step 3. 본문 분량 및 구성 - '체류 시간' 확보]
                        - **목표 분량: 공백 포함 3,000자 이상.** (기사 내용을 단순히 요약하지 말고, TMI와 감상을 덧붙여라)
                        - **문단 구성:** 각 소제목(content) 당 **반드시 줄바꿈(\\n\\n)을 사용하여 2~3개의 문단**으로 나눌 것. (가독성 필수)
                        - **내용 확장:** 1. 기사의 육하원칙을 상세하게 묘사.
                            2. 연예인의 과거 에피소드, 네티즌 반응("댓글 반응"), MBTI 등 TMI 대방출.

                        [Step 4. 섹션별 가이드]
                        * 섹션 1 (도입): 키워드 포함한 첫인사 + 기사를 접한 충격적인 감정 서술.
                        * 섹션 2 (팩트): 기사 내용을 현장감 있게 전달. 핵심 숫자(금액 등)를 구체적으로 명시.
                        * 섹션 3 (심화): 기사 속 상황을 독자가 눈앞에 보듯 생생하게 묘사.
