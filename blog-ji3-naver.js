...
async function writeBlog({
  ...
  try {
    ...
    logWithTime('이미지 업로드 실패 (버튼을 못 찾았거나 파일 문제):', '❌')
  }
  ...
  if (Array.isArray(content)) {
    ...
    for (const section of content) {
      ...
      if (section.content) {
        const sanitizedContent = section.content.replace(/\n\n+/g, '\n');
        await frame.type(contentSpanSelector, sanitizedContent, { delay: 10 });
  ...
  // 발행 세팅
  if (dryRun) {
    console.log(`[DRY-RUN] 발행 과정을 생략하고 스크린샷을 저장합니다: ${title}`);
    const screenshotPath = '/Users/hyeonwoo/.gemini/antigravity-ide/brain/329c64ab-056e-4e65-8e28-7841714a08df/editor_result.png';
    await page.screenshot({ path: screenshotPath }).catch(e => console.error('Screenshot failed:', e));
    console.log(`[DRY-RUN] Screenshot captured at: ${screenshotPath}`);
    return;
  }
...
(async () => {
  ...
  // await checkExecutionTime('m2_time_check.json', 2);
  ...
  logWithTime('시작');
  await naverLogin(page, process.env.NAVER_ID_HS, process.env.NAVER_PW_HS);
  logWithTime('로그인 완료');

  // news.json에서 로커엘 있는거 데이터 읽기
  const newsList = JSON.parse(fs.readFileSync('./data/ji3_data.json', 'utf-8'));

  // 외부 URL에서 newsList 데이터 가져오기 (github raw)
  // const NEWS_JSON_URL = 'https://raw.githubusercontent.com/ggpt6choi-coder/blogman/main/data/ji3_data.json';
